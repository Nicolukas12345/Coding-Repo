...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth){
  function calculatePercentage (number){
    return ((number/100)*percentLossByMonth);
  }
  let debt = (startPriceNew * -1) + calculatePercentage(startPriceNew);
  let month = 0;
  while (debt < 0){
    if (startPriceNew === startPriceOld){
      ...
      debt = startPriceOld - startPriceNew;
      month = 0;
      break;
    };
    month += 1;
    ...
    startPriceOld = startPriceOld - calculatePercentage(startPriceOld);
    startPriceNew = startPriceNew - calculatePercentage(startPriceNew);
    debt = (startPriceNew * -1) + startPriceOld + (month * savingperMonth); 
  }
  return [month, Math.round(debt)];
}
