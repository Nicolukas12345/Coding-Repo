...
//PREP
/*
P: integer array. always positive
R: positive integer
E: {1, 100, 50, -51, 1, 1} output: 1
P: findEvenIndex(arr)
    loop number of arr
      begining = arr.slice(0, index).reduce()
      ending = arr.slice(index + 1, arr.length + 1).reduce
      if (beginning === ending) {return index}
*/
...
function findEvenIndex(arr) {
    ...
    for (let i = 0; i < arr.length; i++){
      beginning = arr.slice(0, i).reduce((a, b) => a + b, 0);
      ending = arr.slice(i + 1, arr.length + 1).reduce((a, b) => a + b, 0);
      console.log (i ?? -1);
      if (beginning === ending) {return i}
    }
    return -1;
}
